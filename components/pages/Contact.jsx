...
const ContactInfo = memo(({ icon, value, href }) => (
  <motion.a
    href={href}
...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!publicKey || publicKey.includes('your_emailjs') || publicKey.includes('here')) {
        throw new Error("NEXT_PUBLIC_EMAILJS_PUBLIC_KEY is missing or using placeholder.");
      }
      if (!serviceId || serviceId.includes('your_emailjs') || serviceId.includes('here')) {
        throw new Error("NEXT_PUBLIC_EMAILJS_SERVICE_ID is missing or using placeholder.");
      }
      if (!templateId || templateId.includes('your_emailjs') || templateId.includes('here')) {
        throw new Error("NEXT_PUBLIC_EMAILJS_TEMPLATE_ID is missing or using placeholder.");
      }
      ...
      if (error.message) {
        if (error.message.includes("VITE_EMAILJS")) {
          ...
          if (error.text.includes("Invalid template ID")) {
            userMessage += "Please check your NEXT_PUBLIC_EMAILJS_TEMPLATE_ID in your .env file.";
          } else if (error.text.includes("Invalid service ID")) {
            userMessage += "Please check your NEXT_PUBLIC_EMAILJS_SERVICE_ID in your .env file.";
          } else if (error.text.includes("Invalid public key")) {
            userMessage += "Please check your NEXT_PUBLIC_EMAILJS_PUBLIC_KEY in your .env file.";
          } else {
  ...
  return (
    <section
      id="contact"
      className="relative min-h-screen bg-linear-to-b from-white to-gray-50 flex items-center justify-center py-20 overflow-hidden"
    >
      {/* Optimized Background Pattern */}
      <div className="absolute inset-0 opacity-[0.015] pointer-events-none">
        <div className="absolute inset-0" style={{
          backgroundImage: `radial-linear(circle at 1px 1px, rgb(0 0 0) 1px, transparent 0)`,
        ...
        <AnimatePresence mode="wait">
          {!showForm ? (
            ...
            >
              ...
              {/* Main Heading with linear text - SMALLER SIZE */}
              <motion.div variants={itemVariants} className="space-y-3">
                <h2 className="text-4xl sm:text-5xl md:text-6xl font-bold leading-tight">
                  <span className="bg-linear-to-r from-red-600 via-red-500 to-pink-600 bg-clip-text text-transparent">
                    Let&apos;s Work Together
                  </span>
                </h2>
                ...
                <motion.div
                  className="w-16 h-1 bg-linear-to-r from-red-600 to-pink-600 mx-auto rounded-full"
                  initial={{ width: 0 }}
                  animate={{ width: 64 }}
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-6">Fill out the form below and I&lsquo;ll get back to you soon.</p>
